...
const { google } = require('./thirdParty_setup')
...
passport.use(new GoogleStrategy({
    ...
    console.info(chalk.blue("Google profile: "), profile);
    ...
    User.findOne(searchQuery, function(err, user) {
        if(err) {
            ...
            //Found google provider data and update old google data to keep data up to date               
            if(user.third_party_auth != null || user.third_party_auth.length > 0){   
                console.log("Found user, update google datas and log in")
                user.third_party_auth.map((provider, index) => {
                    if(provider.provider_name != null && provider.provider_name === 'google') {
